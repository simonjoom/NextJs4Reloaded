...
window.__NEXT_LOADED_PAGES__.forEach(({ route, fn,mod}) => {
  pageLoader.registerPage(route, fn,mod)
})
...
async function doRender ({ Component, props, hash, err, emitter: emitterProp = emitter }) {
  ...
  const appProps = { Component, hash, err, router, headManager,HotAppContainer, ...props }

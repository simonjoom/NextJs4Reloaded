...
/*
 ...
 var cof=[dev && reactJsxSourceItem].filter(Boolean);
 preset.options.plugins=cof.concat(preset.options.plugins);
 // }
...
export default async function getBaseWebpackConfig (dir, {dev = false, isServer = false, buildId, config}) {
  ...
  const clientEntries = !isServer ? {
    'main.js': [dev && !isServer && path.join(__dirname, '..', '..', 'client', 'fixwebpackpath'),
